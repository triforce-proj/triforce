...
contract PYU_NFT_Pool is PyukumukuTokenWrapper, Ownable {
	ERC1155Tradable public pyu;
	...
	constructor(ERC1155Tradable _pyuAddress, IERC20 _pyukumukuAddress) public PyukumukuTokenWrapper(_pyukumukuAddress) {
		pyu = _pyuAddress;
	}
	...
	function redeem(uint256 card) public updateReward(msg.sender) {
		...
		require(pyu.totalSupply(card) < pyu.maxSupply(card), "Max cards minted");

		points[msg.sender] = points[msg.sender].sub(cards[card]);
		pyu.mint(msg.sender, card, 1, "");
