...
contract TRIFORCE_POOLS is Ownable {
    ...
    uint256 public blockRewardPercentage = 100; // The percentage used for triforcePerBlock calculation.

    uint256 public stakingFeeRate = 500; // FeeRate 5%
    uint256 public protocolFeeRate = 0; // FeeRate 0%
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            ...
            if (pending > 0) {
                claimedRewards = claimedRewards.add(pending);
    ...
    function setBlocksPerDay(uint256 _blocksPerDay) external onlyOwner {
        require(_blocksPerDay >= 0 && _blocksPerDay <= 50000, "Value is outside of range 0-50000");
        blocksPerDay = _blocksPerDay;
    }

    function setBlockRewardPercentage(uint256 _blockRewardPercentage) external onlyOwner {
        require(_blockRewardPercentage >= 0 && _blockRewardPercentage <= 100, "Value is outside of range 0-100");
        blockRewardPercentage = _blockRewardPercentage;
    }

    function setTriforcePerBlock(uint256 _triforcePerBlock) external onlyOwner {
        triforcePerBlock = _triforcePerBlock;
    }
