...
contract TRIFORCE_POOLS is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
		...
		// TRIFORCE tokens to distribute per block.
    ...
    uint256 private triforcePerBlock; // TRIFORCE tokens distributed per block. 
	// Use getTriforcePerBlock() to get the updated reward.
    ...
    function withdraw(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (_amount > 0) {
            ...
            uint256 fee = _amount.mul(stakingFeeRate).div(1e4);
    ...
    function safeTokenTransfer(address _to, uint256 _amount) internal {
        ...
        require(triforce.transfer(_to, amount), "Safe token transfer failed");
    }
