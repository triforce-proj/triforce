...
// File: contracts/PYUK/strategies/Common/StratManager.sol
...
contract StratManager is Ownable, Pausable {
     ...
     * @param _poolRewardsAddress address where to send pYUK's fees.
     */
    constructor(
        address _keeper,
        address _strategist,
    ...
    /**
     * @dev Updates pYUK fee recipient.
...
// File: contracts/PYUK/strategies/Common/FeeManager.sol
...
abstract contract FeeManager is StratManager {
    ...
    uint public buyBackFee = 222;
    uint public rewardsFee = MAX_FEE - STRATEGIST_FEE - callFee - buyBackFee;
    ...
    function setCallFee(uint256 _fee) external onlyManager {
        ...
        callFee = _fee;
        rewardsFee = MAX_FEE - STRATEGIST_FEE - callFee - buyBackFee;
    }
}

// File: contracts/PYUK/strategies/Cake/StrategyCakeLP.sol
...
contract StrategyCakeLP is StratManager, FeeManager, GasThrottler {
    ...
    constructor(
        ...
        address _strategist,
        address _poolRewardsAddress
    ) StratManager(_keeper, _strategist, _unirouter, _vault, _poolRewardsAddress) public {
        want = _want;
    ...
    function chargeFees() internal {
        ...
        uint256 rewardsPoolAmount = wbnbBal.mul(rewardsFee).div(MAX_FEE);
        IERC20(wbnb).safeTransfer(poolRewardsAddress, rewardsPoolAmount);

        uint256 buyBackAmounts = wbnbBal.mul(buyBackFee).div(MAX_FEE).div(2);

        IUniswapRouterETH(unirouter).swapExactTokensForTokens(buyBackAmounts, 0, wbnbToPyukRoute, keeper, now.add(600));
        IUniswapRouterETH(unirouter).swapExactTokensForTokens(buyBackAmounts, 0, wbnbToTFCRoute, keeper, now.add(600));
