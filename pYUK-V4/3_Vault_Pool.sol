...
contract PYUKUMUKU is Context, IBEP20, Ownable {	
    ...
    uint256 public _burnFee = 500; // 5% of every transaction's pYUK tokens will be burned 
    ...
    uint256 public _swapCallerFee = 100e18;   // 100 pYUK tokens will be given to the caller of the transaction initiating automatic liquidity generation
    ...
    uint256 public minTokensBeforeSwap = 5000e18; // Contract's pYUK token balance must have a minimum of 5000 pYUK Tokens for automatic liquidity generation
	...
	require(sellingEnabled || owner == ownerWallet || spender == ownerWallet, "Approval is locked.");
    ...
    function setEnabledSelling(bool enabled) external onlyOwner() {	
...
contract PYUK_VAULT_POOLS is Ownable {
    using SafeMath for uint256;
    ...
    uint256 public blockRewardUpdateCycle = 365 days; // The cycle in which the pyukumukuPerBlock gets updated.
    ...
    uint256 public blockRewardPercentage = 680; // The percentage used for pyukumukuPerBlock calculation.

    uint256 public stakingFeeRate = 500; // FeeRate
    ...
    function getPyukumukuPerBlock() public view returns (uint256, bool) {
        ...
        if (block.timestamp >= getPyukumukuPerBlockUpdateTime() || pyukumukuPerBlock == 0) {
            return (poolReward.mul(blockRewardPercentage).div(10000).div(blocksPerDay), true);
        }
    ...
    function add(
        ...
        require(_allocPoint >= 0 && _allocPoint <= 10, "_allocPoint is outside of range 0-10");
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        if (_amount > 0) {
            ...
            pool.stakingToken.safeTransfer(treasuryWallet, fee);
            pool.stakingToken.safeTransfer(address(msg.sender), amountAfterFee);
    ...
    function setBlocksPerDay(uint256 _blocksPerDay) external onlyOwner {
        require(_blocksPerDay >= 0 && _blocksPerDay <= 40000, "Value is outside of range 1000-40000");
        blocksPerDay = _blocksPerDay;
    }

    function setBlockRewardPercentage(uint256 _blockRewardPercentage) external onlyOwner {
        require(_blockRewardPercentage >= 0 && _blockRewardPercentage <= 10000, "Value is outside of range 0-10000");
        blockRewardPercentage = _blockRewardPercentage;
    }
    
    function setPyukPerBlock(uint256 _pyukumukuPerBlock) external onlyOwner {
        pyukumukuPerBlock = _pyukumukuPerBlock;
    }
